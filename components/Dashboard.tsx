...
// ─── CONFIG ────────────────────────────────────────────────
// API calls go through /api/ac proxy — no secrets in the browser
const SDR_GROUP = "1";
...
async function apiFetch(path: string): Promise<Record<string, unknown>> {
    // Encode path + its own querystring into /api/ac?path=...&rest
...
interface Deal {
    ...
    status: string;
    stage: string;
    _cf: Record<string, string>;
}

interface CfEntry { dealId: string; customFieldId: string; fieldValue: string; }

async function fetchAllDeals(group: string, daysBack = 180): Promise<Deal[]> {
    ...
    while (all.length < total) {
        ...
        ((data.dealCustomFieldData as CfEntry[]) || []).forEach(cf => {
            if (!cfMap[cf.dealId]) cfMap[cf.dealId] = {};
            cfMap[cf.dealId][cf.customFieldId] = cf.fieldValue;
        });
        deals.forEach(d => { d._cf = cfMap[d.id] || {}; });
        all.push(...deals);
        total = parseInt(String((data.meta as Record<string, unknown>)?.total ?? all.length));
...
async function fetchFieldMeta(): Promise<Record<string, string>> {
    const data = await apiFetch("/dealCustomFieldMeta?limit=100");
    const map: Record<string, string> = {};
    ((data.dealCustomFieldMeta as Array<{ fieldLabel: string; id: string }>) || []).forEach(f => {
...
async function fetchStages(): Promise<Record<string, string>> {
    ...
    const map: Record<string, string> = {};
    ((data.dealStages as Array<{ id: string; title: string }>) || []).forEach(s => {
        map[s.id] = s.title;
    });
    return map;
}
...
const daysSince = (d: Date | null, ref = new Date()) =>
...
function computeMetrics(
    ...
    const closer = closerDeals.filter(d => (d._cf[FQ] || "") !== TRAINING_MOTIVE);

    // SDR VOLUME TREND
    const weekCounts: Record<string, number> = {};
    sdrDeals.forEach(d => { const k = weekKey(d.cdate); weekCounts[k] = (weekCounts[k] || 0) + 1; });
    const sdrWeeks = Object.entries(weekCounts)
        .sort(([a], [b]) => a.localeCompare(b)).slice(-9)
        .map(([k, n]) => {
            const dt = new Date(k); const end = new Date(dt); end.setDate(dt.getDate() + 6);
    ...
    const sdrThisWeek = sdrDeals.filter(d => inRange(parseDate(d.cdate), lastMonday, lastSunday)).length;
    ...
    prev4weeks.forEach(d => { const k = weekKey(d.cdate); weeklyVols[k] = (weeklyVols[k] || 0) + 1; });
    ...
    // SDR QUAL RATE
    const closerThisWeek = closer.filter(d => inRange(parseDate(d.cdate), lastMonday, lastSunday)).length;
    ...
    const mm4s = daysAgo(28); const mm4e = today;
    const mm4prev_s = daysAgo(56); const mm4prev_e = daysAgo(28);
    const wonInPeriod = (s: Date, e: Date) =>
        closer.filter(d => { const md = parseDate(d.mdate || d.cdate); return d.status === "0" && inRange(md, s, e); });
    const lostInPeriod = (s: Date, e: Date) =>
        closer.filter(d => { const md = parseDate(d.mdate || d.cdate); return d.status === "2" && inRange(md, s, e); });

    const won_curr = wonInPeriod(mm4s, mm4e);
    const lost_curr = lostInPeriod(mm4s, mm4e);
    const open_curr = closer.filter(d => { const cd = parseDate(d.cdate); return d.status === "1" && inRange(cd, mm4s, mm4e); });
    ...
    // VELOCITY
    const enteredMM4 = closer.filter(d => inRange(parseDate(d.cdate), mm4s, mm4e));
    ...
    const convTrend = rollingWindows.map(w => {
        const w_won = wonInPeriod(w.s, w.e); const w_lost = lostInPeriod(w.s, w.e);
    ...
    lost_curr.forEach(d => { const m = (d._cf[FL] || "Não informado").trim(); lossMap[m] = (lossMap[m] || 0) + 1; });
    const lossReasons = Object.entries(lossMap).sort(([, a], [, b]) => b - a).slice(0, 8)
        .map(([motivo, n]) => ({ motivo: motivo.length > 25 ? motivo.slice(0, 23) + "…" : motivo, n, pct: parseFloat((n / (lost_curr.length || 1) * 100).toFixed(1)) }));
    ...
    openDeals.forEach(d => { const name = stageMap[d.stage] || `Stage ${d.stage}`; stageCount[name] = (stageCount[name] || 0) + 1; });
    const pipeByStage = Object.entries(stageCount).sort(([, a], [, b]) => b - a).map(([stage, n]) => ({ stage, n }));

    // COHORT
    const cohortCurr = closer.filter(d => inRange(parseDate(d.cdate), daysAgo(28), daysAgo(14)));
    const cohortPrev = closer.filter(d => inRange(parseDate(d.cdate), daysAgo(45), daysAgo(28)));
    ...
    const sdrQualTrend = sdrWeeks.slice(-5).map(sw => {
        const wStart = new Date(sw.key); const wEnd = new Date(sw.key); wEnd.setDate(wEnd.getDate() + 7);
        const closerInWeek = closer.filter(d => inRange(parseDate(d.cdate), wStart, wEnd)).length;
    ...
    return {
        ...
        sdrWeeks, qualRate: parseFloat(qualRate.toFixed(1)), qualStatus, sdrQualTrend, closerThisWeek,
        conv_curr: parseFloat(conv_curr.toFixed(1)), conv_prev: parseFloat(conv_prev.toFixed(1)), convStatus,
        convTrend, histRate: parseFloat(histRate.toFixed(1)),
        velocity: parseFloat(velocity.toFixed(1)),
        velocityStatus: velocity < 60 ? "red" : velocity < 80 ? "orange" : "green",
        won_curr: won_curr.length, lost_curr: lost_curr.length, open_curr: open_curr.length,
        enteredMM4: enteredMM4.length, lossReasons, openDeals: openDeals.length,
        pipeByAge, pipeByStage, coh1, coh2,
...
// ─── UI HELPERS ────────────────────────────────────────────
const statusColor = (s: string) => ({ green: T.green, orange: T.orange, red: T.red }[s] || T.muted);
const statusIcon = (s: string) => ({ green: "●", orange: "◐", red: "○" }[s] || "○");

function KpiCard({ label, value, sub, status, delta }: { label: string; value: string | number; sub?: string; status: string; delta?: string }) {
    const c = statusColor(status);
    return (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "18px 20px", position: "relative", overflow: "hidden", flex: 1, minWidth: 150 }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: c, borderRadius: "12px 12px 0 0" }} />
            <div style={{ fontSize: 10, color: T.muted, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600, marginBottom: 8 }}>{label}</div>
            <div style={{ fontSize: 30, fontWeight: 800, color: T.white, fontFamily: "Georgia, serif", letterSpacing: "-0.02em", lineHeight: 1 }}>{value}</div>
            ...
            <div style={{ position: "absolute", bottom: 12, right: 14, fontSize: 20, color: c, opacity: 0.7 }}>{statusIcon(status)}</div>
        </div>
    );
}
...
function SectionTitle({ children, tag }: { children: React.ReactNode; tag?: string }) {
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: T.white, letterSpacing: "0.06em", textTransform: "uppercase" }}>{children}</span>
            {tag && <span style={{ fontSize: 10, fontWeight: 700, color: T.bg, background: tag.includes("CRÍTICO") ? T.red : tag.includes("ATENÇÃO") ? T.orange : T.green, borderRadius: 20, padding: "2px 9px", letterSpacing: "0.04em" }}>{tag}</span>}
        </div>
    );
}
...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ color: string; name: string; value: number }>; label?: string }) {
    ...
    return (
        <div style={{ background: "#231740", border: `1px solid ${T.border}`, borderRadius: 8, padding: "10px 14px", fontSize: 12, color: T.cream }}>
            <div style={{ color: T.gold, fontWeight: 700, marginBottom: 4 }}>{label}</div>
            {payload.map((p, i) => (
                <div key={i} style={{ color: p.color }}>{p.name}: <strong>{p.value}{p.name?.includes("Taxa") || p.name?.includes("taxa") ? "%" : ""}</strong></div>
            ))}
        </div>
    );
}
...
export default function Dashboard() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            setError({ type: msg.includes("Failed to fetch") || msg.includes("CORS") ? "cors" : "api", msg });
        ...
        <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "0 28px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", maxWidth: 1200, margin: "0 auto", padding: "16px 0" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <button id="btn-refresh" onClick={loadData} disabled={loading}
            ...
            <div style={{ display: "flex", gap: 2, maxWidth: 1200, margin: "0 auto" }}>
                {tabs.map(t => (
                    <button key={t.id} id={`tab-${t.id}`} onClick={() => setTab(t.id)} style={{ background: "transparent", border: "none", borderBottom: tab === t.id ? `2px solid ${T.gold}` : "2px solid transparent", color: tab === t.id ? T.gold : T.muted, padding: "8px 18px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{t.label}</button>
                ))}
            </div>
        </div>
    );
    ...
    if (error) return (
        <div style={{ background: T.bg, minHeight: "100vh", color: T.white, fontFamily: "'Trebuchet MS', sans-serif", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, padding: 32 }}>
                ...
                <button onClick={loadData} style={{ background: T.berry, border: "none", borderRadius: 8, padding: "10px 24px", color: T.white, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Tentar novamente</button>
            </div>
        </div>
    );
    ...
    const Overview = () => (
        <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                <KpiCard label="Leads SDR (sem.)" value={m.sdrThisWeek} sub={`Média 4 sem: ${m.sdrAvg4}`} status={m.sdrStatus} delta={`${m.sdrVsAvg > 100 ? "▲" : "▼"} ${m.sdrVsAvg.toFixed(0)}% vs média`} />
                <KpiCard label="Taxa Qualificação SDR" value={`${m.qualRate}%`} sub="Range ideal: 10–15%" status={m.qualStatus} delta={m.qualRate < 10 ? "🔴 Restritiva" : m.qualRate > 20 ? "🔴 Alta demais" : "✓ Normal"} />
                <KpiCard label="Conversão Closer MM4s" value={`${m.conv_curr}%`} sub={`Hist. calc: ${m.histRate}%`} status={m.convStatus} delta={`${m.conv_curr > m.conv_prev ? "▲" : "▼"} ${Math.abs(m.conv_curr - m.conv_prev).toFixed(1)}pp vs anterior`} />
                <KpiCard label="Velocity do Pipeline" value={`${m.velocity}%`} sub="Meta: >60%" status={m.velocityStatus} delta={m.velocity >= 60 ? "✓ Deals se movendo" : "🔴 Deals travados"} />
                <KpiCard label="Pipeline Ativo" value={m.openDeals} sub={`${m.pipeByStage[0]?.stage || "—"}: ${m.pipeByStage[0]?.n || 0}`} status={m.pipelineStatus} delta={`${m.won_curr} won · ${m.lost_curr} lost (4 sem)`} />
            </div>
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                        <AreaChart data={m.sdrWeeks} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                            <defs><linearGradient id="sg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.gold} stopOpacity={0.3} /><stop offset="95%" stopColor={T.gold} stopOpacity={0} /></linearGradient></defs>
                            ...
                            {m.sdrAvg4 > 0 && <ReferenceLine y={m.sdrAvg4} stroke={T.rose} strokeDasharray="4 4" label={{ value: `Média ${m.sdrAvg4}`, fill: T.rose, fontSize: 9, position: "insideTopLeft" }} />}
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={m.convTrend} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                            ...
                            <YAxis tick={{ fontSize: 9, fill: T.muted }} tickLine={false} axisLine={false} tickFormatter={v => v + "%"} domain={[0, Math.max(60, ...m.convTrend.map(d => d.taxa)) + 10]} />
                            <Tooltip content={<CustomTooltip />} />
                            {m.histRate > 0 && <ReferenceLine y={m.histRate} stroke={T.gold} strokeDasharray="4 4" label={{ value: `Hist ${m.histRate}%`, fill: T.gold, fontSize: 9, position: "insideTopLeft" }} />}
                            <ReferenceLine y={20} stroke={T.red} strokeDasharray="3 3" label={{ value: "20%", fill: T.red, fontSize: 9, position: "insideBottomRight" }} />
                            <Bar dataKey="taxa" name="Taxa %" radius={[4, 4, 0, 0]}>{m.convTrend.map((d, i) => <Cell key={i} fill={d.taxa < 20 ? "#A03030" : d.taxa < 25 ? T.orange : T.green} />)}</Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
                    {[
                        ...
                        <div key={i} style={{ background: T.surface, borderRadius: 10, border: `1px solid ${statusColor(item.status)}33`, padding: "14px", textAlign: "center" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                        <AreaChart data={m.sdrWeeks} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                            <defs><linearGradient id="sg2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={T.gold} stopOpacity={0.25} /><stop offset="95%" stopColor={T.gold} stopOpacity={0} /></linearGradient></defs>
                            <CartesianGrid strokeDasharray="3 3" stroke={T.border} vertical={false} />
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                        <BarChart data={m.sdrQualTrend} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                            ...
                            <YAxis tick={{ fontSize: 9, fill: T.muted }} tickLine={false} axisLine={false} tickFormatter={v => v + "%"} domain={[0, 30]} />
                            <Tooltip content={<CustomTooltip />} />
                            ...
                            <Bar dataKey="taxa" name="Taxa qualificação" radius={[4, 4, 0, 0]}>{m.sdrQualTrend.map((d, i) => <Cell key={i} fill={d.taxa < 10 ? T.red : d.taxa > 20 ? T.orange : T.rose} />)}</Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                        <BarChart data={m.convTrend} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
                            ...
                            <Tooltip content={<CustomTooltip />} />
                            {m.histRate > 0 && <ReferenceLine y={m.histRate} stroke={T.gold} strokeDasharray="4 4" label={{ value: `Histórico ${m.histRate}%`, fill: T.gold, fontSize: 9, position: "insideTopLeft" }} />}
                            ...
                            <Bar dataKey="taxa" name="Taxa %" radius={[5, 5, 0, 0]} maxBarSize={70}>{m.convTrend.map((d, i) => <Cell key={i} fill={d.taxa < 20 ? "#A03030" : d.taxa < 25 ? T.orange : T.green} />)}</Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </div>
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                        ...
                        {[
                            ...
                            <div key={i}>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 12, color: T.muted }}>
                                    <span>{row.label}</span><span style={{ color: row.color, fontWeight: 700 }}>{row.v}</span>
                                </div>
            ...
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    {[{ label: "Cohort Atual (14–28 dias atrás)", c: m.coh1 }, { label: "Cohort Anterior (29–45 dias atrás)", c: m.coh2 }].map(({ label, c }, i) => (
                        <div key={i} style={{ background: T.surface, borderRadius: 10, border: `1px solid ${c.rate < 20 ? T.red : T.border}44`, padding: "16px 18px" }}>
                            ...
                            <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
                                {([["Won", c.won, T.green], ["Lost", c.lost, T.red], ["Open", c.open, T.orange]] as [string, number, string][]).map(([l, v, color]) => (
                            ...
                            <div style={{ fontSize: 12, color: T.muted }}>Taxa atual: <span style={{ color: c.rate < 20 ? T.red : T.green, fontWeight: 700 }}>{c.rate.toFixed(1)}%</span> ({c.won}/{c.won + c.lost} decididos)</div>
                        </div>
                    ))}
                </div>
            </div>
                            ...
                            return (
                                <div key={i}>
                                    ...
                                    <div style={{ height: 7, background: T.border, borderRadius: 4 }}>
                                        <div style={{ height: "100%", width: `${(st.n / (m.pipeByStage[0]?.n || 1)) * 100}%`, background: c, borderRadius: 4 }} />
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: "20px 22px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 4 }}>
                        {m.pipeByAge.map((ag, i) => {
                            ...
                            return (
                                <div key={i} style={{ background: T.surface, borderRadius: 8, border: `1px solid ${c}33`, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: T.white }}>{ag.label}</div>
                                        <div style={{ fontSize: 10, color: c, marginTop: 2 }}>{ag.status === "green" ? "alta chance" : ag.status === "orange" ? "maturando" : "baixa chance"}</div>
                                    </div>
                                    <div style={{ textAlign: "right" }}>
                                        <div style={{ fontSize: 26, fontWeight: 800, color: ag.n === 0 ? T.muted : c, fontFamily: "Georgia, serif" }}>{ag.n}</div>
                                        <div style={{ fontSize: 10, color: T.muted }}>{m.openDeals > 0 ? Math.round(ag.n / m.openDeals * 100) : 0}%</div>
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
                    {(() => {
                        ...
                        return [
                            ...
                            <div key={i} style={{ background: T.surface, borderRadius: 10, border: `1px solid ${p.color}44`, padding: "16px 18px" }}>
                                ...
                                <div style={{ fontSize: 11, color: T.muted, marginTop: 4 }}>{p.n !== "–" && `${p.n} deals · `}{p.note}</div>
                            </div>
                        ));
                    })()}
                </div>
